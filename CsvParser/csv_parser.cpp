...
void csv_parser::_get_fields_with_optional_enclosure(csv_row_ptr row, const char * line, const unsigned int * line_length)
{
	...
	if (*line_length > 0)
	{
		...
		register unsigned int field_end			= 0U;
		register unsigned int char_pos 			= 0U;
		register unsigned int enclosed_state	= 0U;
		...
		while(char_pos < *line_length)
		{
			...
			else if (curr_char == field_term_char &&
				!enclosed_state)
			{
				field_end = char_pos;
				...
				char_pos == *line_length - 1)
			{
				field_end = char_pos;
				if (char_pos == *line_length - 1)
				{
					++field_end;
				}
				while (line[field_end - 1] == enclosed_char)
				{
					--field_end;
				}
